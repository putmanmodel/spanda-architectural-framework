...
The paper series is organized under `papers/` in three groups:

- `papers/core/` — foundational Spanda architecture papers (`Paper1`–`Paper9`)
- `papers/companion/` — companion and framing papers (`PaperCompanion1`–`PaperCompanion8`)
- `papers/bridges/` — bridge papers extending the architecture into new domains (`PaperBridge1`, etc.)

This structure separates core architectural foundations from companion notes and controlled cross-domain applications while keeping the series under one repository.
...
Papers 1–3 define what agents may infer, how they persist, and how they destabilize collectively. Paper 4 defines how deviation is detected and governed before collapse propagates. Papers 5–7 define optional representation, reflex, and entry-contract layers that improve interoperability and real-time stability without creating new prerequisites. Papers 8–9 formalize normative and reference-level integration constraints for conformance and replayable stack wiring.
...
Companion papers support the main Spanda series through framing notes, clarification papers, and focused extensions that help interpret and apply the core architecture.

They are distinct from the foundational sequence (`Paper1`–`Paper9`) and from bridge papers, which extend the architecture into new domains.

These papers are organized under `papers/companion/`.
...
### PaperBridge1
**Weak-Signal Interpretation for Low-Altitude Drone Detection: Toward a Stratified, Deviation-Governed Sensing Architecture**  
Zenodo: https://doi.org/10.5281/zenodo.15189938

A bridge paper extending the Spanda / PUTMAN weak-signal architecture into defensive low-altitude small-drone sensing.

**Key contributions:**
- Reframes low-altitude small-drone detection as a weak-signal interpretation problem rather than a single-stage classification task  
- Separates Sensor Partials, Reflex Salience, Deviation Evaluation, Candidate State, and Governed Promotion into an explicit layered pipeline  
- Adds a minimal architecture contract defining inputs, outputs, time horizons, and forbidden transitions across layers  
- Provides a toy simulation specification for evaluating persistence, cross-channel support, bearing variance, and promotion outcomes  
- Extends the broader Spanda architecture into a simulation-oriented defensive sensing domain without introducing interception or engagement logic  
...
Future revisions may expand formalization and simulation validation; this series establishes the architectural foundation while companion and bridge papers extend its interpretive and domain-facing applications.
